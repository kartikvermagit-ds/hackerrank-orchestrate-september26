...
# 2. EventNormalizer with shopping in fixed categories, and outlier filter in living expenses
...
def patched_calculate_variable_daily_burn(events, request_date, recurring_categories=None):
    req_dt = pd.to_datetime(request_date)
    start_dt = req_dt - pd.Timedelta(days=60)
    
    debits = [
        e for e in events 
        if e.status == 'settled' and e.direction == 'debit' and e.category in norm.LIVING_EXPENSE_CATEGORIES 
        and start_dt <= pd.to_datetime(e.event_date) < req_dt
    ]
    if not debits:
        return 0.0
    
    # Filter out extreme outliers (> 3x category average or bulk purchases)
    amounts = []
    for e in debits:
        desc_l = e.description.lower()
        if 'bulk' in desc_l and 'purchase' in desc_l:
            continue
    ...
    return sum(amounts) / 60.0

norm.calculate_variable_daily_burn = patched_calculate_variable_daily_burn

cb = ContextBuilder(p, e, o, norm)
fc = ForecastEngine()
sc = SafeAmountCalculator(fc)

# 3. Patch compute_earliest_date_for_full_payment to use 90-day horizon
